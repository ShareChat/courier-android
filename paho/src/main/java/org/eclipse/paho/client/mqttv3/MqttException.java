...
public class MqttException extends Exception
{
	...
	/**
	 * The client is currently disconnecting and cannot accept any new work. This can occur when waiting on a token, and then disconnecting the client. If the message delivery does
	 * not complete within the quiesce timeout period, then the waiting token will be notified with an exception.
	 */
	...
	/**
	 * The client is not connected to the server. The {@link MqttClient#connect()} or {@link MqttClient#connect(MqttConnectOptions)} method must be called first. It is also
	 * possible that the connection was lost - see {@link MqttClient#setCallback(MqttCallback)} for a way to track lost connections.
	 */
	public static final short REASON_CODE_CLIENT_NOT_CONNECTED = 32104;

	/**
	 * Server URI and supplied <code>SocketFactory</code> do not match. URIs beginning <code>tcp://</code> must use a <code>javax.net.SocketFactory</code>, and URIs beginning
	 * <code>ssl://</code> must use a <code>javax.net.ssl.SSLSocketFactory</code>.
	 */
	...
	/**
	 * Thrown when an attempt to call {@link MqttClient#disconnect()} has been made from within a method on {@link MqttCallback}. These methods are invoked by the client's thread,
	 * and must not be used to control disconnection.
	 * 
	...
	/**
	 * Protocol error: the message was not recognized as a valid MQTT packet. Possible reasons for this include connecting to a non-MQTT server, or connecting to an SSL server port
	 * when the client isn't using SSL.
	 */
	public static final short REASON_CODE_INVALID_MESSAGE = 32108;

	/**
	 * The client has been unexpectedly disconnected from the server. The {@link #getCause() cause} will provide more details.
	 */
	...
	/**
	 * A request has been made to use a token that is already associated with another action. If the action is complete the reset() can ve called on the token to allow it to be
	 * reused.
	 */
	public static final short REASON_CODE_TOKEN_INUSE = 32201;

	/**
	 * A request has been made to send a message but the maximum number of inflight messages has already been reached. Once one or more messages have been moved then new messages
	 * can be sent.
	 */
	 ...
	 */
	public static final int REASON_CODE_SUBSCRIPTION_NOT_ACK = 128;
	 ...
	 * 
	 * @return the Throwable that was the root cause of this exception, which may be <code>null</code>.
	 */

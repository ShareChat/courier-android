...
public class MqttConnectOptions
{
	...
	/**
	 * The default handshake timeout in seconds if one is not specified
	 */
	public static final int HANDSHAKE_TIMEOUT_DEFAULT = 30;

	/**
	 * The default read timeout in seconds if one is not specified
	 */
	...
	private int keepAliveIntervalServer = KEEP_ALIVE_INTERVAL_DEFAULT;
	...
	private int handshakeTimeout = HANDSHAKE_TIMEOUT_DEFAULT;
	 ...
	 * @return the handshake timeout value.
	 */
	 ...
	 * be established. The default timeout is 30 seconds. A value of 0 disables timeout processing meaning the client will wait until the network connection is made successfully or
	 * fails.
	 * 
	...
	/**
	 * Sets the read timeout value. This value, measured in seconds, defines the maximum time interval a read() call on the InputStream associated with this Socket will block for.
	 * The default timeout is 360 seconds. A value of 0 is interpreted as an infinite timeout.
	 *
	 * @param readTimeout
	 *            the timeout value, measured in seconds. It must be >0;
	 */
	 ...
	 * properties are <em>not</em> used if a SocketFactory has been set using {@link #setSocketFactory(SocketFactory)}. The following properties can be used:</p>
	 * <dl>
	 ...
	 * <ul>
	 * <li>If set to false both the client and server will maintain state across restarts of the client, the server and the connection. As state is maintained:
	 * <ul>
	 * <li>Message delivery will be reliable meeting the specified QOS even if the client, server or connection are restarted.
	 * <li>The server will treat a subscription as durable.
	 * </ul>
	 * <lI>If set to true the client and server will not maintain state across restarts of the client, the server or the connection. This means
	 * <ul>
	 ...
	 * <p>
	 * Each <code>serverURI</code> specifies the address of a server that the client may connect to. Two types of connection are supported <code>tcp://</code> for a TCP connection
	 * and <code>ssl://</code> for a TCP connection secured by SSL/TLS. For example:
	 * <ul>
	 ...
	 * <p>
	 * When an attempt to connect is initiated the client will start with the first serverURI in the list and work through the list until a connection is established with a server.
	 * If a connection cannot be made to any of the servers then the connect attempt fails.
	 * <p>
	 ...
	 * <p>
	 * Some MQTT servers support a high availability feature where two or more "equal" MQTT servers share state. An MQTT client can connect to any of the "equal" servers and be
	 * assured that messages are reliably delivered and durable subscriptions are maintained no matter which server the client connects to.
	 * <p>
	 * The cleansession flag must be set to false if durable subscriptions and/or reliable message delivery is required.
	 * <li>Hunt List
	 * <p>
	 * A set of servers may be specified that are not "equal" (as in the high availability option). As no state is shared across the servers reliable message delivery and durable
	 * subscriptions are not valid. The cleansession flag must be set to true if the hunt list mode is used
	 * </ol>
	 * </p>
	 * 
	 * @param array

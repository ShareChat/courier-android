...
/**
 * Represents a persistent data store, used to store outbound and inbound messages while they are in flight, enabling delivery to the QoS specified. You can specify an
 * implementation of this interface using {@link MqttClient#MqttClient(String, String, MqttClientPersistence)}, which the {@link MqttClient} will use to persist QoS 1 and 2
 * messages.
 * <p>
 * If the methods defined throw the MqttPersistenceException then the state of the data persisted should remain as prior to the method being called. For example, if
 * {@link #put(String, MqttPersistable)} throws an exception at any point then the data will be assumed to not be in the persistent store. Similarly if {@link #remove(String)}
 * throws an exception then the data will be assumed to still be held in the persistent store.
 * </p>
 * <p>
...
{
	/**
	 * Initialise the persistent store. If a persistent store exists for this client ID then open it, otherwise create a new one. If the persistent store is already open then just
	 * return. An application may use the same client ID to connect to many different servers, so the client ID in conjunction with the connection will uniquely identify the
